...
export function createSequelize6Instance(connectionString: string, options?: Sequelize6Options): Sequelize6;
export function createSequelize6Instance(options: Sequelize6Options): Sequelize6;
export function createSequelize6Instance(config: string | Sequelize6Options, options?: Sequelize6Options): Sequelize6 {
  if (typeof config === 'string') {
    return new Sequelize6(config, wrapOptions(options));
  }

  return new Sequelize6(wrapOptions(config));
